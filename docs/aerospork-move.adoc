...
[verse]
// tag::synopsis[]
aerospork move [-h|--help] [--window-id <window-id>] [--boundaries <boundary>] [--boundaries-action <boundary-action>] (left|down|up|right)
...
// tag::body[]
{manpurpose}. See the "Examples" section for more details.
...
the implicit container is not created.
Instead, `window 2` is moved to the monitor above the current one.
