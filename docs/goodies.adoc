...
Do you have a cool automatization, AeroSpork integration, or workflow?
...
You can also use `after-startup-command` to start JankyBorders together with AeroSpork
...
It drives the `aerospace` binary, so it needs the command name changed to `aerospork` to work here.
...
Here are a few third party options in alphabetical order. All of them were written for upstream
AeroSpace, so they invoke the `aerospace` binary and read `AEROSPACE_*` environment variables --
both need adjusting for AeroSpork:
...
You can integrate AeroSpork workspace indicators with link:https://github.com/FelixKratz/SketchyBar[Sketchybar].
...
Opening *a new window* of a program that can supports multiple windows (such as Safari or Terminal.app) can be accomplished with an AppleScript inlined in `aerospork.toml` as follows:
