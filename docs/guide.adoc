...
. Put unpacked `aerospork-v$VERSION/bin/aerospork` anywhere to `$PATH`
(The step is optional. It is only needed if you want to be able to interact with AeroSpork from CLI)
...
the bash, fish and zsh completions. You may need to configure your shell to pick up Homebrew's
...
AeroSpork tries to find the custom config in two locations:
...
AeroSpork uses TOML format for the config.
...
If no custom config is found, AeroSpork will load the default config.
...
`start-at-login = false` (all four shown, commented out, in `default-config.toml`), plus three the
default config does not mention at all: `enable-normalization-flatten-containers = true`,
`enable-normalization-opposite-orientation-for-nested-containers = true` and
`auto-move-workspaces-on-monitor-connect = true`. `after-startup-command`,
`on-focus-changed`, `on-focused-workspace-changed`, `on-focused-monitor-changed`,
`on-window-detected` and `workspace-to-monitor-force-assignment` default to empty; the `exec` table
defaults as described under <<exec-env-vars>>.

So the safe way to start is to copy the default config and edit it, rather than to write a short
file and assume the rest is inherited:
...
The initial binding mode that AeroSpork starts out with is "main".
...
Commands is the thing that you use to manipulate AeroSpork and query its state.
...
* For `dvorak` and `colemak` users, AeroSpork offers preconfigured presets.
+
[source,toml]
----
[key-mapping]
...
* `exec.inherit-env-vars = true` configures whether inherit environment variables of `aerospork.app` or not. (The default is `true`)
...
including any secrets in it. That is deliberate — a callback that posts to Slack needs its token —
but it means a config snippet copied from the internet runs with everything your shell has. Set
...
That's why unless you override `exec` section in your config, AeroSpork fallbacks to the following `exec` configuration:
+
[source,toml]
----
[exec]
...
In total, AeroSpork provides 4 possible layouts:
...
Since Spaces are so hard to deal with, AeroSpork reimplements Spaces and calls them "Workspaces".
...
When you quit the AeroSpork or when the AeroSpork detects that it's about to crash, AeroSpork will place all windows back to the visible area of the screen.

AeroSpork shows the name of currently active workspace in its tray icon (top right corner), to give users a visual feedback on what workspace is currently active.

The intended workflow of using AeroSpork workspaces is to only have one macOS Space (or as many monitors you have, if `Displays have separate Spaces` is enabled) and don’t interact with macOS Spaces anymore.
...
That means, that if AeroSpork crashes badly you will still be able to manually "unhide" the windows by dragging these few pixels to the center of the screen.
...
Since AeroSpork needs a free space to hide windows in,
...
For some reason, mission control doesn't like that AeroSpork puts a lot of windows in the bottom right corner of the screen.
...
moving windows between monitors causes windows to move between different Spaces which is not correctly handled by the public APIs AeroSpork uses,
...
|👍 Public Apple API are more stable (which in turn affects AeroSpork stability)
...
* You can use xref:commands.adoc#list-apps[`aerospork list-apps`] CLI command to get IDs of running applications
...
When you switch to an empty workspace, AeroSpork puts the workspace on an assigned monitor; i3 puts the workspace on currently active monitor.

* I find that AeroSpork model works better with <<observation,_the observation_>> listed above.
* AeroSpork model is more consistent (it works the same for empty workspaces and non-empty workspaces)
====
...
# Fingerprint by vendor and model. Both are numbers -- as a TOML integer, or as a '0x'-prefixed
# hex string. A bare 'D0C1' or a vendor *name* like 'LG' is a parse error, not a match failure.
...
When `auto-move-workspaces-on-monitor-connect` is enabled (default), AeroSpork will automatically move workspaces to their assigned monitors when monitors are connected. This ensures your workspace layout is restored when docking or connecting external displays.
...
* `display_name` - Monitor name (string). Matched case-insensitively: exact first, then as a
  substring. It is **not** a regex -- the bare-string, non-fingerprint form of a monitor pattern is,
  but this field is not, so `'LG|DELL'` here matches a display literally named `LG|DELL` and nothing
  else.
...
AeroSpork uses the API to gently ask windows whether they are dialogs, but AeroSpork also applies some heuristics.
...
It's fine to hardcode special handling for popular applications, AeroSpork already does it.
Please see `isDialogHeuristic` function in AeroSpork sources.
...
If you can't make AeroSpork handle some keys in your config, please make sure that you don't have keys conflict with other software that might listen to global keys (e.g. skhd, Karabiner-Elements, Raycast)
...
turned on: the events worth diagnosing after the fact -- which config file loaded, why one didn't,
config warnings, the CLI socket coming up, and any CLI command that exited non-zero -- are always
recorded.
...
* These records are written at debug level, which the unified log **does not persist**. They exist
  ...
  So start the stream first, then reproduce the problem.
