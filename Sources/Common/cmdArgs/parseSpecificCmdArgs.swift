  ...
  while !args.isEmpty {
    ...
    if arg == "-h" || arg == "--help" {
      ...
      if let optionParser: any ArgParserProtocol<T> = T.parser.options[arg] {
        ...
        // Name the flags this command does accept: the user is already in the right
        // command and only needs to see the nearby spelling.
        let known = T.parser.options.keys.sorted()
        errors.append(
          "Unknown flag \(arg.singleQuoted)\(didYouMeanSuffix(arg, from: known))" +
            (known.isEmpty ? "" : "\nSupported flags: \(known.joined(separator: ", "))")
        )
        break
      }
