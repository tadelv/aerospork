...
extension AxUiElementMock {
    ...
    func isDialogHeuristic(appBundleId id: String?) -> Bool {
        ...
        //
        // KNOWN FALSE POSITIVE. Repro: put one Firefox window into non-native fullscreen (F11);
        // every OTHER Firefox window then reports a disabled minimize button and is misclassified
        // as floating. The heuristic cannot tell "this window is a PiP panel" from "some sibling
        // window is fullscreen" through AX alone. Resolving it needs a signal that distinguishes
        // them -- PiP has no `AXCloseButton` either, which is the most likely candidate, but it
        // has not been captured in `axDumps/` yet, so it cannot be tested headlessly today.
