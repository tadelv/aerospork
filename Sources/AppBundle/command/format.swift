            ...
            case (.monitor(let m), .monitor(let f)):
                return switch f {
                    ...
                    case .monitorFingerprint:
                        if let fingerprint = m.fingerprint {
                            .success(.string(fingerprint.description))
                        } else {
                            .success(.string(""))
                        }
                    case .monitorVendorId:
                        if let fingerprint = m.fingerprint, let vendorId = fingerprint.vendorID {
                            .success(.string(String(format: "0x%04X", vendorId)))
                        } else {
                            .success(.string(""))
                        }
                    case .monitorModelId:
                        if let fingerprint = m.fingerprint, let modelId = fingerprint.modelID {
                            .success(.string(String(format: "0x%04X", modelId)))
                        } else {
                            .success(.string(""))
                        }
                    case .monitorSerialNumber:
                        if let fingerprint = m.fingerprint, let serial = fingerprint.serialNumber {
