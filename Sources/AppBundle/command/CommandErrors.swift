...
/// A no-op report, with the tip only when the user has not already taken it. Suggesting
/// `--fail-if-noop` to someone who just passed `--fail-if-noop` reads like the command did not see
/// the flag.
func noopMessage(_ message: String, failIfNoop: Bool) -> String {
  failIfNoop ? message : "\(message) \(noopTip)"
}
...
extension Command {
  ...
  func handleNoop(_ message: String, failIfNoop: Bool, io: CmdIo) -> Bool {
    io.err(noopMessage(message, failIfNoop: failIfNoop))
    return !failIfNoop
  }
