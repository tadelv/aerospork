...
func swapWindows(_ window1: Window, _ window2: Window) {
    ...
    // BOTH operands, not window1 twice. The second guard used to read `window1.ownIndex` again --
    // a copy-paste that left window2 completely unchecked, so swapping with an unbound window hit
    // `unbindFromParent()`'s `dieT("... is already unbound")` and took the app down. Reachable:
    // `moveTilingWindow` picks its swap target from `lastAppliedLayoutPhysicalRect`, which is stale
    // by construction -- it is written by the previous layout pass, and a window can be garbage
    // collected between that pass and the mouse-up that reads it.
    guard window1.isBound, window2.isBound else { return }

    // One unbind order, not two mirrored ones. `unbindFromParent()` reports the index at the moment
    // of removal, so the second unbind already accounts for the first -- both branches of the
    // `index1 < index2` fork this replaces placed the windows identically (checked for both
    // orderings within one parent, and across two parents). They differed only in which window
    // `bind` marked most-recent, and this is the order that already ran every time.
