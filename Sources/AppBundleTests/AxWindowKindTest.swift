...
extension [String: Json]: AxUiElementMock {
    ...
    /// Captured `axDumps/` fixtures are read-only by construction. Tests that need to exercise the
    /// AX write path use `FakeAxElement` (AxWriteTest.swift) instead.
    public func set<Attr>(_ attr: Attr, _ value: Attr.T) -> Bool where Attr: WritableAttr {
        dieT("axDumps fixtures are read-only; use FakeAxElement to test AX writes")
    }
